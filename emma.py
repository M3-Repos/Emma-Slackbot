...
class Emma(App):
    def __init__(self, token, user_memories, ai_client, rate_limit=10):
    ...
    def init_events(self):
        ...
        def handle_app_mentions(body, say):
            ...
            if current_time < self.allowed_time:
                time_remaining = self.allowed_time - current_time
                logger.info(
                    f"Rate limited, try request again after {int(time_remaining)} seconds"
                )
                say(
                    f"Rate limited, try request again after {int(time_remaining)} seconds"
                )
            ...
            # Process request only if not rate limited
            ...
            # Update allowed time after successful processing
            self.allowed_time = current_time + self.rate_limit
